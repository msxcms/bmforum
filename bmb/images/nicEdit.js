...
var nicEditorConfig = bkClass.extend({
	...
	bmbcode : false,
	editorid : 1,
	buttonList : ['save','bold','italic','underline','left','center','right','justify','ol','ul','fontSize','fontFamily','indent','outdent','image','upload','link','unlink','forecolor','bgcolor','xhtml','emot'],
	iconList : {"xhtml":1,"bgcolor":2,"forecolor":3,"bold":4,"center":5,"hr":6,"indent":7,"italic":8,"justify":9,"left":10,"ol":11,"outdent":12,"removeformat":13,"right":14,"save":25,"strikethrough":16,"subscript":17,"superscript":18,"ul":19,"underline":20,"image":21,"link":22,"unlink":23,"close":24,"arrow":26,"upload":27,"emot":28}
...
var nicEditorInstance = bkClass.extend({
	...
	init : function() {
		...
		this.elm.setAttribute('id','bmfEditor'+this.ne.options.editorid);	
		if(this.getContent() == "") {
			this.setContent('<br />');
		}
	...
	saveContent : function() {
		if(this.copyElm || this.options.hasPanel) {
			...
			if(this.ne.options.bmbcode) {
				(this.copyElm) ? this.copyElm.value = FTB_FilterScript(this.getContent()) : this.e.innerHTML = FTB_FilterScript(this.getContent());
			} else {
...
var nicEmotOptions = {
	buttons : {
		'emot' : {name : 'Emotion', type : 'nicEmotButton'}
	}
	
};
/* END CONFIG */

var nicEmotButton = nicEditorAdvancedButton.extend({
	width : '350px',
	emotion: true,
	...
	addPane : function() {
		...
		this.myFrame = new bkElement('div').setAttributes({ id: 'Layer2' }).setStyle({border : 0}).appendTo(this.pane.pane);
		$("#Layer2").html(emotionList);
	},
...
nicEditors.registerPlugin(nicPlugin,nicEmotOptions);

function FTB_FilterScript(str) { // BMB Code 
	str = str.replace(/\n+/g,"");
	str = str.replace(/\r/g,"");
	...
	str = str.replace(/<img[^>]+src="([^"]+)"[^>]+bmbemotion="([^ >]+)"[^>]*>/ig,"$2");
	...
	str = str.replace(/<pre[^>]*>(.*?)<\/pre>/ig,"[quote]$1[/quote]");
	str = str.replace(/<([\/]?)td[^>]+bgcolor="([^ >]+)"([^>]*)*>/ig,"[$1td=$2]");
	str = str.replace(/<([\/]?)th[^>]+bgcolor="([^ >]+)"([^>]*)*>/ig,"[$1td=$2]");
...
function FTB_InsertText(text, editorID) {
    var sel, range, html;
    editorID = "bmfEditor" + (editorID ? editorID : 1);
    if (!$.browser.msie) {
