...
$aquery = "SELECT * FROM {$database_up}threads WHERE tid='$topid' LIMIT 0,1";
...
get_forum_info("");
// ######## 检测是否为管理员开始 ##########
$query = "SELECT * FROM {$database_up}forumdata WHERE id='$forum_cid'";
$result = bmbdb_query($query);
while (false !== ($adline = bmbdb_fetch_array($result))) {
...
if ($login_status == 1 && $check_user == 0 && $adminlist != "") {
    ...
    for ($i = 0; $i < $admincount; $i++) {
        if ($arrayal[$i] == $username && $arrayal[$i] != "" && $arrayal[$i] != "|") $announceadmin = 1;
    } 
} 
// ######## 检测是否为管理员结束 ##########
$msgg_max = 100; #系统记录的最大区置顶数目
$pagesize = 10; #每页显示数目
if (empty($job)) {
...
if ($ctop_true != "1") $announceadmin = 0;
...
if ($job == "write") {
    ...
    if ($count_pint["$cateid"] >= $max_qzd) error_page($gl[230], $gl[475], $gl[475], $ggl[49]);
}

if (empty($process)) {
    include("header.php");
    navi_bar($gl[230], $gl[477]);
    print_confirm();
    include("footer.php");
    exit;
} 
...
if ($job == "write") {
    if ($announceadmin != 1) {
    	error_page($gl[230], $gl[477], $gl[477], $ggl[18]);
    }
    if ($process == 2) {
        ...
        if ($nline['toptype'] == 9) update_pincache($topid, 1, 0);

        $query = "UPDATE {$database_up}threads SET toptype=8 WHERE tid='$topid' LIMIT 1";
        $result = bmbdb_query($query);
        
        update_pincache($topid, 0, $cateid);
        
        // log this action
        if ($beforeactionmess == "yes") {
            mtou($nline['author'], $job, $nline['title']);
        } 

        $showinfo = "{$nline['title']}({$nline['author']})";
        $nquery = "insert into {$database_up}actlogs (actdetail,acter,actreason,acttime,forumid,actioncode) values ('$showinfo','$username','$actionreason','$timestamp','{$nline['forumid']}','cate$job')";
        $result = bmbdb_query($nquery);
        //finish

        jump_page("forums.php?forumid=$foruid", "$ggl[45]",
            "<strong>$ggl[46]</strong><br /><br />
...
if ($job == "delone") {
    ...
    if ($nline['type'] >= 3) $toptype = 1; else $toptype = 0;

    $query = "UPDATE {$database_up}threads SET toptype='$toptype' WHERE tid='$topid' LIMIT 1";
    $result = bmbdb_query($query);
    
    update_pincache($topid, 1, $cateid);

    jump_page("forums.php?forumid=$nline[forumid]", "$ggl[44]",
            "<strong>$ggl[44]</strong><br /><br />
			$gl[3] <a href='forums.php?forumid=$nline[forumid]'>$gl[4]</a> | <a href='topic.php?forumid=$nline[forumid]&filename=$topid'>$gl[8]</a> | <a href='index.php'>$gl[5]</a>", 3);

} 

function print_confirm()
{
    global $foruid, $topid, $log_hash, $author, $cateid, $choose_reason, $username, $article, $job, $gl;
    $title = "$gl[173]";
    $chooser_t = explode("\n", $choose_reason);
    $cou = count($chooser_t);
